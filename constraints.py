...
class IfAllThenOneConstraint(Constraint):
    ...
    def __init__(self, name, left_side, right_side, left_values, right_values):
        ...
        self._ls = left_side  # Left side variables
        self._rs = right_side  # Right side variables
        self._lv = left_values  # Allowed values for left side variables
        self._rv = right_values  # Allowed values for right side variables

    def check(self):
        """Check whether the constraint is satisfied."""
        # Check if all variables on the left side are assigned and equal left_values
        left_assigned = all(var.isAssigned() for var in self._ls)
        if left_assigned and all(var.getValue() in self._lv for var in self._ls):
            # At least one variable in the right side must match right_values
            right_assigned = any(var.isAssigned() and var.getValue() in self._rv for var in self._rs)
            return right_assigned
        # If not all left-side variables meet the condition, the constraint is satisfied
        return True

    def hasSupport(self, var, val):
        """Return True if var=val has a valid assignment for this constraint."""
        # Temporarily assign var to val and check consistency
        var.setValue(val)
        is_valid = self.check()
        var.unAssign()
        return is_valid
